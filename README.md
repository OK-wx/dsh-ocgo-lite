...
- Client 轮询携带 `?live=<sessionId>`，Host 并行直读**最近活跃会话**的日志文件
  （多帧 zstd 解压 + JSONL 行解析，秒级），替换缓存条目后重聚合全局 →
  **本次会话与全部范围下的模型明细均实时**
- 内存保护：只扫最近活跃 **8** 个会话、单文件 ≤ **8MB** 压缩（解压文本 ≤ 128MB）、
  并发 ≤ **2**；全量基线 5 分钟缓存 + in-flight 锁（防重复扫描），冷启动一次性
