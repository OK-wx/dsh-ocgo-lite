...
```
[GO:]  滚动：◯5%  周：◯7%  月：◯3%  模型：全部  token：14.5M  花费：$1.54
```
...
| **模型：** | 点击弹出模型选择器——所有消耗过的模型 + 各自 token/花费，选中后状态条 token/花费联动 |
| **token** | 完整数字 + 详情卡片（输入/输出/推理/缓存读/写 + 按模型分组明细） |
| **花费** | 详情卡片：累计金额 + 按模型花费排行（占比条 + 各模型官方定价） |

- 点击页面空白处关闭详情卡片
- 30 秒自动刷新（无感，不闪 loading）
- 切换会话立即显示（前端共享缓存 + Host 10 分钟统计缓存）
- 复制成功有 toast 弹窗提示
...
| 数据 | 来源 | 口径 |
|---|---|---|
| **配额余量** | 官方 `https://opencode.ai/zen/go/v1/usage`（Bearer auth.json key） | **账户级**（含其他设备/软件） |
| **token / 花费** | DSH 会话事件（`sessionQuery`，`assistant/message` 的 usage，过滤 opencode-go provider） | **仅 DSH 会话**（不含 opencode CLI 等） |
...
- `GET /ocgo-lite/api` — 聚合 JSON（配额 + DSH token/花费 + 按模型 + 账户掩码）
