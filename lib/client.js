...
`;
    ...
    function OcgoLite(props) {
      ...
      const curSession = scope === "session"
        ? currentSession
        : scope === "recent"
          ? bsList.reduce((a, b) => (((b.mtime || 0) > ((a && a.mtime) || 0)) ? b : a), null)
          : null;
      ...
      if (popKey === "model") {
        ...
        const scopeVal = (key) => {
          ...
          if (key === "recent") return recentSession ? fmtInt(sumTok(recentSession.tokens)) + " · " + fmtUsd(recentSession.cost) : "暂无记录";
        ...
        const scopeRow = (key, label, sub) => {
          ...
          return React.createElement("div", {
            ...
            onClick: (e) => {
              ...
              if (key !== "all") {
                const target = key === "session"
                  ? (sessionId ? (bsList.find((b) => b.sessionId === sessionId)
                    || bsList.find((b) => b.sessionId === "session-" + sessionId)
        ...
        content = React.createElement(React.Fragment, null,
          ...
          scopeRow("recent", "最近对话", recentSession ? "最近活跃会话的 token 与花费（含子代理/后台任务）" : "暂无用量记录"),
          React.createElement("div", { style: Object.assign({}, rowStyle, { marginTop: 6 }) },
            React.createElement("button", { style: btnStyle, onClick: (e) => { e.stopPropagation(); setPop(null); } }, "收起")));
