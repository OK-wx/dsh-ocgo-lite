...
`;
    ...
    function OcgoLite(props) {
      ...
      // 'recent'=**当前会话**最后一次任务(lastTurn)的执行消耗(含正在执行的任务)
      const curSession = scope === "session"
        ? currentSession
        : scope === "recent"
          ? (() => {
              if (!currentSession || !currentSession.lastTurn) return currentSession || null;
              return { ...currentSession, tokens: currentSession.lastTurn.tokens, cost: currentSession.lastTurn.cost, requests: currentSession.lastTurn.requests, costUnknown: currentSession.lastTurn.costUnknown || 0, byModel: currentSession.lastTurn.byModel };
            })()
          : null;
      ...
      // 徽标 provider:选中模型 → 该模型所属 provider;否则当前活跃 provider(加载时模型选的谁就是谁)
      const actProv = (s && s.activeProvider) || null;
      const selProv = (selModel && selModel.provider) || null;
      const badgeProvider = selProv || actProv || "opencode-go";
      ...
      if (popKey === "model") {
        ...
        const cardUnknown = selModel ? (selModel.costUnknown || 0) : (scoped ? ((curSession && curSession.costUnknown) || 0) : ((s && s.costUnknown) || 0));
