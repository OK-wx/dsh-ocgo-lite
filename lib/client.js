...
`;
    ...
    function OcgoLite(props) {
      ...
      // 注意:curSession 始终尝试匹配(不依赖当前 scope)——范围=全部时,
      // 范围卡片里的"本次会话"行也要显示真实数值,而不是"无记录"。
      ...
      if (popKey === "model") {
        ...
        const cardTok = selModel ? (selModel.tokens || {}) : (scope === "session" ? (curSession ? curSession.tokens : {}) : t);
        const cardTotal = selModel ? sumTok(selModel.tokens) : scopeTotal;
        const cardTitle = selModel
          ? "模型 " + selModel.model
          : (scope === "session" ? "本次会话总消耗" : "总消耗 token");
        ...
        content = React.createElement(React.Fragment, null,
          ...
          rows,
          cardModels.length ? React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: "var(--dsw-alias-label-tertiary, #888)", marginTop: 6 } }, "按模型" + (selModel ? "（仅所选模型）" : (scope === "session" ? "（本次会话）" : ""))) : null,
          modelRows,
          React.createElement("div", { style: footStyle }, "仅统计 DSH 会话（opencode-go 流量）· 范围：" + (scope === "session" ? "本次会话" : "全部") + (selModel ? " · 模型：" + selModel.model : "")),
          React.createElement("div", { style: Object.assign({}, rowStyle, { marginTop: 6 }) },
            React.createElement("button", { style: btnStyle, onClick: (e) => { e.stopPropagation(); load(); } }, loading ? "刷新中…" : "刷新"),
            React.createElement("button", { style: btnStyle, onClick: (e) => { e.stopPropagation(); setPop(null); } }, "收起")),
          React.createElement("div", { style: footStyle }, "更新于 " + fmtClock(lastUpdated) + " · 自动刷新 30 秒"));
        ...
        const cardSessions = selModel ? selModel.requests : (scope === "session" ? (curSession ? 1 : 0) : (s.sessions == null ? "-" : s.sessions));
        ...
        content = React.createElement(React.Fragment, null,
          React.createElement("div", { style: h4Style }, "消耗金额" + (selModel ? " · " + selModel.model : (scope === "session" ? " · 本次会话" : ""))),
          ...
          bm.length ? React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: "var(--dsw-alias-label-tertiary, #888)", marginTop: 6 } }, "按模型（花费）" + (selModel ? "（仅所选模型）" : (scope === "session" ? " · 本次会话" : ""))) : null,
          modelCostRows,
          React.createElement("div", { style: footStyle }, "仅 DSH 会话 · 按官方定价估算 · 范围：" + (scope === "session" ? "本次会话" : "全部") + (selModel ? " · 模型：" + selModel.model : "")),
