...
`;
    ...
    function OcgoLite(props) {
      // 当前会话 ID(来自 composer.dock 槽位 props;用于"本次会话"切换)
      ...
      const curSession = scope === "session" && sessionId ? (
      ...
      // 范围=本次会话时,模型联动/模型选择/明细均基于本次会话的模型列表
      const curModels = (curSession && curSession.byModel) || [];
      const modelList = scope === "session" ? curModels : bmList;
      const selModel = modelSel ? modelList.find((m) => m.model === modelSel) : null;
      // 当前范围口径下的总量/花费(模型"全部"行与明细标题用):全部=全局合计;本次会话无记录则为 0
      const scopeTotal = scope === "session" ? (curSession ? sumTok(curSession.tokens) : 0) : total;
      const scopeCost = scope === "session" ? (curSession ? curSession.cost : 0) : s.cost;
      // 范围+模型双层联动:范围=本次会话时,选中模型则显示该模型在本次会话的量,未选则显示本次会话总量
      const shownTotal = scope === "session"
        ? (curSession ? (selModel ? sumTok(selModel.tokens) : scopeTotal) : 0)
        : (selModel ? sumTok(selModel.tokens) : total);
      const shownCost = scope === "session"
      ...
      const bar = React.createElement("span", { ref: wrapRef, style: { display: "inline-flex", alignItems: "center", gap: 0, whiteSpace: "nowrap", fontSize: 10, color: "var(--dsw-alias-label-secondary, #888)" } },
        ...
        seg("scope", [
          ...
          React.createElement("span", { key: "b", style: { fontWeight: 700, color: "var(--dsw-alias-label-primary, inherit)", fontVariantNumeric: "tabular-nums" } }, scope === "session" ? "本次会话" : "全部"),
        ]),
        React.createElement("span", { style: sepBox }),
        seg("model", [
      ...
      if (popKey === "model") {
        ...
        const modelRow = (label, m) => {
          const mt = m ? (m.tokens || {}) : (scope === "session" ? ((curSession && curSession.tokens) || {}) : t);
        ...
        content = React.createElement(React.Fragment, null,
          React.createElement("div", { style: h4Style }, "选择模型" + (scope === "session" ? " · 本次会话" : "")),
          rows,
        ...
        // 范围选择卡片:全部会话 / 本次会话;选择后状态条 token/花费联动显示
        const scopeVal = (key) => {
          if (key === "session") return curSession ? fmtInt(sumTok(curSession.tokens)) + " · " + fmtUsd(curSession.cost) : "暂无记录";
        ...
        const scopeRow = (key, label, sub) => {
          ...
          return React.createElement("div", {
            ...
            onClick: (e) => {
              e.stopPropagation();
              setScope(key);
              // 切到本次会话时:单模型自动选中该模型;多模型/无记录则回到"全部"
              if (key === "session" && sessionId) {
                const target = bsList.find((b) => b.sessionId === sessionId)
                  || bsList.find((b) => b.sessionId === "session-" + sessionId)
                  || bsList.find((b) => b.sessionId === sessionId.replace(/^session-/, ""));
        ...
        content = React.createElement(React.Fragment, null,
          ...
          scopeRow("session", "本次会话", curSession ? "仅当前聊天会话的 token 与花费" : "当前会话暂无用量记录（显示 0）"),
          React.createElement("div", { style: Object.assign({}, rowStyle, { marginTop: 6 }) },
            React.createElement("button", { style: btnStyle, onClick: (e) => { e.stopPropagation(); setPop(null); } }, "收起")));
        ...
        // 范围=本次会话时,明细/按模型均取本次会话口径(无记录则为 0)
        const st = (scope === "session" ? (curSession ? curSession.tokens : {}) : t);
        const rows = [
          ["输入：", fmtInt(st.input)], ["输出：", fmtInt(st.output)], ["推理：", fmtInt(st.reasoning)],
          ["缓存读：", fmtInt(st.cacheRead)], ["缓存写：", fmtInt(st.cacheWrite)],
        ].map(([k, v]) =>
        ...
        content = React.createElement(React.Fragment, null,
          React.createElement("div", { style: h4Style }, (scope === "session" ? "本次会话总消耗" : "总消耗 token") + "  " + fmtInt(scopeTotal)),
          rows,
          modelList.length ? React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: "var(--dsw-alias-label-tertiary, #888)", marginTop: 6 } }, "按模型" + (scope === "session" ? "（本次会话）" : "")) : null,
          modelRows,
          React.createElement("div", { style: footStyle }, "仅统计 DSH 会话（opencode-go 流量）· 范围：" + (scope === "session" ? "本次会话" : "全部")),
          React.createElement("div", { style: Object.assign({}, rowStyle, { marginTop: 6 }) },
            React.createElement("button", { style: btnStyle, onClick: (e) => { e.stopPropagation(); load(); } }, loading ? "刷新中…" : "刷新"),
            React.createElement("button", { style: btnStyle, onClick: (e) => { e.stopPropagation(); setPop(null); } }, "收起")),
          React.createElement("div", { style: footStyle }, "更新于 " + fmtClock(lastUpdated) + " · 自动刷新 30 秒"));
        ...
        content = React.createElement(React.Fragment, null,
          React.createElement("div", { style: h4Style }, "消耗金额" + (scope === "session" ? " · 本次会话" : "")),
          React.createElement("div", { style: rowStyle }, React.createElement("span", null, "累计："),
            React.createElement("span", { style: bStyle }, fmtUsd(scopeCost) + " 美元")),
          React.createElement("div", { style: rowStyle }, React.createElement("span", null, "会话数："),
            React.createElement("span", { style: bStyle }, scope === "session" ? (curSession ? 1 : 0) : (s.sessions == null ? "-" : s.sessions))),
          bm.length ? React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: "var(--dsw-alias-label-tertiary, #888)", marginTop: 6 } }, "按模型（花费）" + (scope === "session" ? " · 本次会话" : "")) : null,
          modelCostRows,
          React.createElement("div", { style: footStyle }, "仅 DSH 会话 · 按官方定价估算 · 范围：" + (scope === "session" ? "本次会话" : "全部")),
