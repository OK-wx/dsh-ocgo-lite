...
`;
    ...
    function OcgoLite() {
      ...
      // 按模型联动:选中某模型时,状态条 token/花费显示该模型的数值
      const bmList = (s && s.byModel) || [];
      const selModel = modelSel ? bmList.find((m) => m.model === modelSel) : null;
      const shownTotal = selModel
        ? (selModel.tokens.input || 0) + (selModel.tokens.output || 0) + (selModel.tokens.reasoning || 0) + (selModel.tokens.cacheRead || 0) + (selModel.tokens.cacheWrite || 0)
        : total;
      const shownCost = selModel ? selModel.cost : s.cost;
      ...
      let content = null;
      if (popKey === "model") {
        // 模型选择卡片:全部 + 消耗过的模型,每行显示该模型 token 与花费;点击选中并联动状态条
        const modelRow = (label, m) => {
          const mt = m ? (m.tokens || {}) : t;
          const mTotal = (mt.input || 0) + (mt.output || 0) + (mt.reasoning || 0) + (mt.cacheRead || 0) + (mt.cacheWrite || 0);
          const mCost = m ? m.cost : s.cost;
        ...
        bmList.forEach((m) => rows.push(modelRow(m.model, m)));
        content = React.createElement(React.Fragment, null,
          React.createElement("div", { style: h4Style }, "选择模型"),
          rows,
          React.createElement("div", { style: footStyle }, "选择后状态条 token/花费联动显示该模型"),
        ...
        const rows = [
          ["输入：", fmtInt(t.input)], ["输出：", fmtInt(t.output)], ["推理：", fmtInt(t.reasoning)],
          ["缓存读：", fmtInt(t.cacheRead)], ["缓存写：", fmtInt(t.cacheWrite)],
        ].map(([k, v]) =>
        ...
        // 按模型分组(token 明细)
        const bm = (s && s.byModel) || [];
        const modelRows = bm.map((m) => {
          const mt = (m.tokens || {});
          const mTotal = (mt.input || 0) + (mt.output || 0) + (mt.reasoning || 0) + (mt.cacheRead || 0) + (mt.cacheWrite || 0);
        ...
        content = React.createElement(React.Fragment, null,
          React.createElement("div", { style: h4Style }, "总消耗 token  " + fmtInt(total)),
          rows,
          bm.length ? React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: "var(--dsw-alias-label-tertiary, #888)", marginTop: 6 } }, "按模型") : null,
          modelRows,
          React.createElement("div", { style: footStyle }, "仅统计 DSH 会话（opencode-go 流量）"),
          React.createElement("div", { style: Object.assign({}, rowStyle, { marginTop: 6 }) },
            React.createElement("button", { style: btnStyle, onClick: (e) => { e.stopPropagation(); load(); } }, loading ? "刷新中…" : "刷新"),
            React.createElement("button", { style: btnStyle, onClick: (e) => { e.stopPropagation(); setPop(null); } }, "收起")),
          React.createElement("div", { style: footStyle }, "更新于 " + fmtClock(lastUpdated) + " · 自动刷新 30 秒"));
        ...
        content = React.createElement(React.Fragment, null,
          ...
          React.createElement("div", { style: footStyle }, "用户身份（邮箱）需网页端登录：opencode.ai → Sign in"),
          React.createElement("div", { style: Object.assign({}, rowStyle, { marginTop: 6 }) },
            React.createElement("button", { style: btnStyle, onClick: (e) => { e.stopPropagation(); load(); } }, loading ? "刷新中…" : "刷新"),
            React.createElement("button", { style: btnStyle, onClick: (e) => { e.stopPropagation(); setPop(null); } }, "收起")),
          React.createElement("div", { style: footStyle }, "更新于 " + fmtClock(lastUpdated) + " · 自动刷新 30 秒"));
        ...
        content = React.createElement(React.Fragment, null,
          React.createElement("div", { style: h4Style }, "消耗金额"),
          React.createElement("div", { style: rowStyle }, React.createElement("span", null, "本机累计："),
            React.createElement("span", { style: bStyle }, fmtUsd(s.cost) + " 美元")),
          React.createElement("div", { style: rowStyle }, React.createElement("span", null, "会话数："),
            React.createElement("span", { style: bStyle }, s.sessions == null ? "-" : s.sessions)),
          bm.length ? React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: "var(--dsw-alias-label-tertiary, #888)", marginTop: 6 } }, "按模型（花费）") : null,
          modelCostRows,
          React.createElement("div", { style: footStyle }, "仅 DSH 会话 · 按官方定价估算"),
