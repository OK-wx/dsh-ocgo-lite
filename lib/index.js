  ...
  try {
    ...
    for (let k = 0; k < sessions.length; k++) {
      ...
      for (const ev of snap.events) {
        ...
        if (c != null) m.cost += c
        // 按会话分组聚合(供"本次会话"切换);会话内再按模型聚合(供范围联动明细)
        let ss = bySession.get(sid)
        if (!ss) {
          ss = { sessionId: sid, requests: 0, tokens: { input: 0, output: 0, reasoning: 0, cacheRead: 0, cacheWrite: 0 }, cost: 0, byModel: new Map() }
          bySession.set(sid, ss)
        }
        ss.requests++
        ss.tokens.input += ti
        ss.tokens.output += to
        ss.tokens.reasoning += tr
        ss.tokens.cacheRead += crDelta
        ss.tokens.cacheWrite += cw
        if (c != null) ss.cost += c
        let sm = ss.byModel.get(mKey)
        if (!sm) {
          sm = { model: mKey, requests: 0, tokens: { input: 0, output: 0, reasoning: 0, cacheRead: 0, cacheWrite: 0 }, cost: 0 }
          ss.byModel.set(mKey, sm)
        }
        sm.requests++
        sm.tokens.input += ti
        sm.tokens.output += to
        sm.tokens.reasoning += tr
        sm.tokens.cacheRead += crDelta
        sm.tokens.cacheWrite += cw
        if (c != null) sm.cost += c
      }
      if (prevCr.has(sid)) counted++
    }
    stats.sessions = counted
    stats.bySession = Array.from(bySession.values())
      .map((s) => {
        ...
        return { sessionId: s.sessionId, requests: s.requests, tokens: s.tokens, cost: r4(s.cost), byModel: sms }
      })
