...
// 单会话事件扫描:返回该会话聚合(缓存读按会话内相邻增量;byModel 为该会话内模型明细)
function scanSessionEvents(snap, sid) {
  const agg = {
    sessionId: sid,
    requests: 0,
  ...
  if (!snap || !snap.events) return agg
  let prevCr = null
  for (const ev of snap.events) {
    ...
    const crDelta = prevCr == null ? crRaw : Math.max(0, crRaw - prevCr)
    prevCr = crRaw
    ...
    if (c != null) { agg.cost += c; agg.costKnown++; m.cost += c }
  }
  return agg
}
...
// 从 per-session 聚合列表重算全局:token 合计/金额/按模型(含定价)
function aggregateSessions(list) {
  ...
  stats.bySession = list.map((s) => {
    ...
    return { sessionId: s.sessionId, requests: s.requests, tokens: s.tokens, cost: r4(s.cost), costKnown: s.costKnown, byModel: sms }
  })
  for (const s of stats.bySession) {
    ...
    for (const m of s.byModel) {
      let gm = byModel.get(m.model)
      if (!gm) {
        gm = { model: m.model, requests: 0, tokens: { input: 0, output: 0, reasoning: 0, cacheRead: 0, cacheWrite: 0 }, cost: 0 }
        byModel.set(m.model, gm)
      }
  ...
  const run = (async () => {
    ...
    try {
      ...
      for (let k = 0; k < sessions.length; k++) {
        ...
        const agg = scanSessionEvents(snap, sessions[k].header.id)
...
// 实时通道:重读最近活跃的多个会话(按文件 mtime Top N + 指定会话),
// 替换缓存中对应条目后重聚合全局。解决"本次会话/全部+模型"统计滞后:
// 只重读当前会话时,其他会话(子代理/并行会话)的模型流量要等缓存过期。
// 优先直读会话文件(多帧 zstd 解压 + JSONL 行解析,每文件 1~3s),
// 指定会话文件不可读时回退 sessionQuery。
const LIVE_TOP_N = 3
async function liveSessionStats(sq, sessionId) {
  ...
  if (base.error || !sessionId) return base
  try {
    const targets = findRecentSessionLogs(sessionId, LIVE_TOP_N)
    if (!targets.length) return base
    const aggs = []
    for (const t of targets) {
      const text = decompressSessionLog(t.filePath)
      if (!text) continue
      const events = []
      for (const line of text.split('\n')) {
        ...
        try {
          ...
          if (ev && ev.type === 'assistant/message' && ev.data) events.push(ev)
        } catch { /* 跳过坏行 */ }
      }
      if (!events.length) continue
      const agg = scanSessionEvents({ events }, t.sessionId)
      if (agg.requests > 0) aggs.push(agg)
    }
    // 指定会话文件不可读时,回退 sessionQuery(重放校验较慢但保底)
    if (!aggs.length) {
      const sessions = await sq.listSessions()
      let rec = null
      for (const r of sessions) {
        if (sameSession(r.header.id, sessionId)) { rec = r; break }
      }
      if (!rec) return base
      const snap = await sq.readSession(rec.header.id)
      if (!snap || !snap.events) return base
      const agg = scanSessionEvents(snap, rec.header.id)
      if (agg.requests > 0) aggs.push(agg)
    }
...
// 按文件 mtime 找最近活跃的会话日志(含指定会话,去重);目录名兼容两种格式
function findRecentSessionLogs(extraId, topN) {
  const home = homeDir()
  if (!home) return []
  ...
  for (const w of ws) {
    ...
    for (const s of subs) {
      ...
      if (!byId.has(s)) byId.set(s, { sessionId: s, filePath: f, mtime })
    }
  }
  const all = Array.from(byId.values()).sort((a, b) => b.mtime - a.mtime)
  const top = all.slice(0, topN)
  if (extraId) {
    const names = [extraId, 'session-' + extraId, extraId.replace(/^session-/, '')]
    const found = all.find((x) => names.some((n) => x.sessionId === n))
...
// 解压多帧 zstd 会话日志为 JSONL 文本;坏帧直接跳过(不阻断)
