...
// 官方定价(opencode.ai/docs/go, per 1M tokens; 2026-08-16 官方表格,运行时会被官方页面抓取覆盖)
// 峰谷模型取 Peak 档(保守计量);自动抓取时取同模型较贵档,避免被 Off-Peak 覆盖。
let PRICING = {
  'deepseek-v4-flash': { in: 0.44, out: 1.32, cr: 0.014, cw: 0 }, // Peak(Off-Peak: 0.22/0.66/0.007)
  'deepseek-v4-pro': { in: 1.32, out: 3.96, cr: 0.044, cw: 0 }, // Peak(Off-Peak: 0.66/1.98/0.022)
  'mimo-v2.5': { in: 0.14, out: 0.28, cr: 0.0028, cw: 0 },
  'mimo-v2.5-pro': { in: 0.435, out: 0.87, cr: 0.003625, cw: 0 },
  'muse-spark-1.2-contributor': { in: 0.1, out: 0.2, cr: 0.002, cw: 0 },
  ...
  // 历史模型名(旧会话数据)映射:deepseek-chat/reasoner = MiMo V2.5 价;其余保留零兜底
...
async function fetchOfficialPricing() {
  try {
    ...
    // seen:本轮同归一名去重取高(峰谷 Peak/Off-Peak、上下文档档取较高价),
    // 避免 Off-Peak/短档覆盖 Peak/长档导致低估
    const seen = new Map()
    for (const row of rows) {
      ...
      // 官方页面为显示名(如 "DeepSeek V4 Flash" / "GPT 5.6 Luna (≤ 272K tokens)" / "MiMo V2.5 Pro"),
      ...
      const prevSeen = seen.get(name)
      if (prevSeen && priceIn <= prevSeen.in) continue // 同归一名只取较贵档
      seen.set(name, { in: priceIn, out: priceOut, cr: priceCr })
    }
    let found = 0
    for (const [name, p] of seen) {
      // 官方表格模型(含新增 MiMo/Muse 等)全部纳入;未知模型也可自动加入
      const existing = PRICING[name]
      PRICING[name] = {
        in: p.in,
        out: p.out,
        cr: isNaN(p.cr) ? (existing ? existing.cr : 0) : p.cr,
        cw: existing ? existing.cw : 0,
      }
      found++
    }
    if (found > 0) {
...
function aggregateSessions(list) {
  ...
  stats.byModel = Array.from(byModel.values())
    ...
    .sort((a, b) => b.cost - a.cost)
  // 套餐模型清单:PRICING 全量(GO 套餐所有已知模型+定价,含官方新增/未用过的),
  // 供前端模型选择器展示完整清单
  stats.models = Object.entries(PRICING)
    .map(([model, p]) => ({
      model,
      in: p.in,
      out: p.out,
      ...
      cw: p.cw,
      // 是否在本范围用过(用于选择器标记)
      used: stats.byModel.some((m) => m.model === model),
    }))
    .sort((a, b) => (b.used ? 1 : 0) - (a.used ? 1 : 0))
  return stats
}
