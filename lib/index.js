...
// 内存保护:sessionQuery 全量读会把大会话事件全部载入内存,故只取最近 MAX_SCAN_FILES 个会话,并发 2
  ...
  const run = (async () => {
    ...
    try {
      ...
      const recent = sessions
        .slice()
        .sort((a, b) => (b.header?.lastEventAt ?? b.header?.createdAt ?? 0) - (a.header?.lastEventAt ?? a.header?.createdAt ?? 0))
      ...
      await Promise.all(Array.from({ length: Math.min(SCAN_CONCURRENCY, recent.length) }, () => worker()))
      const list = []
      for (let k = 0; k < recent.length; k++) {
        const snap = snaps[k]
        if (!snap || !snap.events) continue
        const agg = scanSessionEvents(snap, recent[k].header.id)
  ...
  try {
    ...
    async function scanWorker() {
      while (fi < changed.length) {
        ...
        const t = changed[i]
        try {
          const text = await decompressSessionLogAsync(t.filePath)
          if (!text) { results[i] = null; continue }
          const events = []
          ...
          if (!events.length) { results[i] = null; continue }
...
// 内存保护:按最后修改时间只取最近 MAX_SCAN_FILES 个,并跳过超大文件
function findAllSessionLogs() {
  ...
  if (out.length > MAX_SCAN_FILES) out.length = MAX_SCAN_FILES
  return out
}
...
// 并行解压多帧 zstd 会话日志为 JSONL 文本;坏帧直接跳过(不阻断)
// 内存保护:读取前检查文件大小,解压后检查文本大小,超大直接跳过
const zstdDecompressAsync = promisify(zstdDecompress)
async function decompressSessionLogAsync(filePath) {
  try {
    let st
    try { st = statSync(filePath) } catch { return '' }
    if (st.size > MAX_SCAN_FILE_BYTES) return ''
    const buf = await readFile(filePath)
    const frames = scanZstdFrames(buf)
    if (!frames.length) return ''
    const parts = []
    for (const f of frames) {
      if (f.end - f.start > MAX_SCAN_FILE_BYTES) return ''
      const out = await zstdDecompressAsync(buf.subarray(f.start, f.end))
      parts.push(out)
      let total = 0
      for (const p of parts) {
        total += p.length
        if (total > MAX_SCAN_TEXT_BYTES) return ''
      }
    }
    return parts.map((b) => b.toString('utf8')).join('')
  } catch { return '' }
}
