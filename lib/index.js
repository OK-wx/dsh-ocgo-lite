...
// 实时通道:只重读指定会话,替换缓存中该会话条目后重聚合全局。
// 优先直读会话文件(多帧 zstd 解压 + JSONL 行解析,秒级),绕过 sessionQuery
// 的重放校验(大会话可达 30s+);文件缺失/解析失败再回退 sessionQuery。
async function liveSessionStats(sq, sessionId) {
  ...
  try {
    let agg = null
    const filePath = findSessionLogFile(sessionId)
    if (filePath) {
      ...
      if (text) {
        ...
        if (events.length) agg = scanSessionEvents({ events }, sessionId)
      }
    }
    if (!agg || agg.requests === 0) {
      ...
      agg = scanSessionEvents(snap, rec.header.id)
    }
    if (!agg || agg.requests === 0) return base
    const list = base.bySession.filter((b) => !sameSession(b.sessionId, agg.sessionId))
    list.push(agg)
...
// 按 sessionId 在 ~/.dsh/sessions/<workspace>/<session-<uuid>|uuid>/ 下找会话日志文件
function findSessionLogFile(sessionId) {
  const home = homeDir()
  if (!home || !sessionId) return null
  const root = join(home, '.dsh', 'sessions')
  if (!existsSync(root)) return null
  const names = [sessionId, 'session-' + sessionId, sessionId.replace(/^session-/, '')]
  let ws
  try { ws = readdirSync(root) } catch { return null }
  for (const w of ws) {
    let st
    try { st = statSync(join(root, w)) } catch { continue }
    if (!st.isDirectory()) continue
    for (const n of names) {
      const f = join(root, w, n, 'session.jsonl.zstd')
      if (existsSync(f)) return f
      const f2 = join(root, w, n, 'session.jsonl')
